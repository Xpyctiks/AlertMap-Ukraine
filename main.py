...
def load_config():
  """Loads the current confifuration from .env file"""
...
def check_state() -> bool:
  """Checks the state of LEDs because they may be totally off."""
  try:
...
def get_day_night() -> int:
  """Gets current time of the day - day or night, to set proper brightness"""
  #if not enabled, just return brightness value of the day
  if not DAY_NIGHT_ENABLED:
    return DAY_NIGHT_DAYBRIGHTNESS
  #if enabled
  now = datetime.now().time()
  if (now >= time(DAY_NIGHT_FROM, 0) or now < time(DAY_NIGHT_TO, 0)):
    return DAY_NIGHT_NIGHTBRIGHTNESS
  else:
    return DAY_NIGHT_DAYBRIGHTNESS

def set_state(led_id: str, color: list = [0,0,255]) -> bool:
...
def reset_brightness() -> None:
  """Deletes memory file at the night start and night end time to set all LEDs to the proper brightness"""
  now = datetime.now().time()
  if (time(DAY_NIGHT_FROM, 0) <= now <= time(DAY_NIGHT_FROM, 1)) or (time(DAY_NIGHT_TO, 0) <= now <= time(DAY_NIGHT_TO, 1)):
...
def main():
  ...
  try:
    ...
    if len(response) != len(regions_api_map):
      print(f"Неверная длина ответа: {len(response)} != {len(regions_api_map)}")
      raise ValueError(f"Неверная длина ответа: {len(response)} != {len(regions_api_map)}")
    ...
    for region_id,status in enumerate(response,0):
      ...
      if MEMORY_UPDATED and status == memory[region_id] and LEDS_ARE_OK:
        ...
        if status == "A":
          if not set_state(regions.get(regions_api_map[region_id]),RED):
            set_state_errors = 1
        elif status == "P":
          if not set_state(regions.get(regions_api_map[region_id]),YELLOW):
            set_state_errors = 1
        else:
          if not set_state(regions.get(regions_api_map[region_id]),GREEN):
            set_state_errors = 1
    ...
    if set_state_errors != 0:
      asyncio.run(send_to_telegram(f"set_state() errors appeared during the operations! Check logs!",f"🚒AlarmMap-Ukraine update script:"))
